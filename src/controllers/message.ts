...
export const send_text: RequestHandler = async (req, res) => {
  try {
    const { number, message: m, options } = req.body;
    const session = getSession(req.body.key)!;
    ...
    if (!exists) return res.status(400).json({ error: 'Number does not exists' });
